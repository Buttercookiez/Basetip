...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV6dmN6Y21vaWh5cnV6c3ZjaXNtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ2NjI3ODAsImV4cCI6MjA3MDIzODc4MH0.hZkW-YzKL6MxORxHwE5Zu1A19_Mb9LhM0YfXZ2xHQ40";
...
async function loadTip() {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error || !data) {
    statusEl.textContent = "Tip link not found";
    return;
  }
  threadBox.innerHTML =
    '<a href="' +
...
async function checkNetwork() {
  ...
  if (chainId !== 84532) {
    alert("Please switch to Base Sepolia to send tips.");
    return false;
  }
  return true;
}
...
connectBtn.addEventListener("click", async () => {
  ...
  const baseChainIdHex = "0x14a34"; // 84532 in hex
  try {
...
sendBtn.addEventListener("click", async () => {
  ...
  if (!recipientAddress) return alert("No valid recipient loaded");
  ...
  try {
    ...
    statusEl.innerHTML =
      `🚀 Transaction sent: <a href="https://sepolia.basescan.org/tx/${tx.hash}" target="_blank">${tx.hash}</a>`;

    const receipt = await tx.wait();
    statusEl.innerHTML += `<br>✅ Confirmed in block ${receipt.blockNumber}`;
...
async function loadTxHistory() {
  ...
  if (error) {
    txHistoryEl.textContent = "Failed to load history";
    return;
  }
  if (!data || data.length === 0) {
    txHistoryEl.textContent = "No transactions yet";
    return;
  }

  txHistoryEl.innerHTML = data
    .map(
      (t) => `
